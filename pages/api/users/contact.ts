...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    if (req.method === "POST") {
        try {
            ...
            const sendingMessage = emailContactFormMessage(contactForm.name, contactForm.email, contactForm.subject, contactForm.message);
            const statusCode = await sendEmail(sendingMessage);

            return res.status(201).json({
                status: 201,
                message: "Thank you for writing to me. I'll get back to you in few days.",
            });
        } catch (error: any) {
